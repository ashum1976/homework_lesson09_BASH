...
Перевая --  запускает генератор лог-файла, для проверки анализатором, каждую минут, и всё находится под управлением flock утилиты, предотвращающей запуск второго такого процесса.
Вторая -- запускает сам анализатор лог-файла, формирует и отсылает почту пользователю vagran. Всё тоже под управлением flock утилиты.

___
___

##          Текст скриптов для выполнения ДЗ
...
<details> 
    ...
    #Запускаем почтовый сервер postfix, для приёма сообщения для root
    systemctl start postfix
    
    #Расположение крон-файла пользователя root
    ...
    mutt -s "Анализ лог файла ${filelog}, за период ${mail_log_time}" root@localhost < ${filedata}report_stat_file.txt && rm ${filedata}report_stat_file.txt
