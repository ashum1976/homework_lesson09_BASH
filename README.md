...
-  **Ctrl+a Ctrl+e                 :** <----- *Возврат курсора в начало строки, переход курсора в конец строки*

-  **Alt+f Alt+b                    :**  <----- _Перемещение на слово вперёд, премещение на слово назад_

-  **Ctrl+w                           :**   <---- _Ctrl+w - удаляет слово до курсора (при этом копируя его в буфер обмена)_

-  **Alt+d                              :** <----- _Удаляет слово после курсора_   

-  **Ctrl+u                            :** <---- _Удаляет всю строку до курсора_

-  **Ctrl+k                            :** <----   _Удаляет всю строку после курсора_
...
- **Ctrl-x Ctrl-e**	Запустить редактор и после его закрытия выполнить набранные в нём команды.
...
- **Ctrl-x ~**	Вывести возможное дополнение именами пользователей. Такой же результат при нажатии ~+tab+tab
...
- **Meta-@**	Дополнить начатое слово до возможного варианта имени хоста из /etc/hosts.
- **Ctrl-x @**	Вывести возможные варианты хостов. Берутся значения из файла /etc/hosts
...
- **!abc**         Повторить последнюю команду начинающуюся с "abc"
- **!abc:p**       Напечатать последнюю команду начинающуюся с "abc"
- **!$**           Последний аргумент предыдущей команды
- **!\***          Все аргументы предыдущей команды
- **^abc^321**     Запустить предыдущую команду, заменв в ней "abc" на "321"
...
Для того, чтобы получить список всех открытых в Linux дескрипторов, можно воспользоваться командой **lsof** . Для того, чтобы узнать PID текущего процесса, можно использовать специальную переменную окружения $$ , в которую оболочка записывает текущий PID. Ключ **"-a"** используется для выполнения операции логического **И** над результатами, возвращёнными благодаря использованию двух других ключей:

      [andrey@nix64amd Vim]$ lsof -a -p $$ -d 0-255

      COMMAND     PID   USER   FD   TYPE DEVICE SIZE/OFF      NODE NAME
      bash    3555558 andrey    0u   CHR 136,29      0t0        32 /dev/pts/29
      bash    3555558 andrey    1w   CHR 136,29      0t0        32 /dev/pts/29
      bash    3555558 andrey    2u   CHR 136,29      0t0        32 /dev/pts/29
      bash    3555558 andrey    3w   REG   8,33     2260 156912179 /storage2/Study/Vim/stdout2_test
      bash    3555558 andrey    6w   CHR 136,29      0t0        32 /dev/pts/29
      bash    3555558 andrey   12w  FIFO   0,13      0t0 160517821 pipe
      bash    3555558 andrey   14w  FIFO   0,13      0t0 160517822 pipe
      bash    3555558 andrey  255u   CHR 136,29      0t0        32 /dev/pts/29
...
Cmnd_Alias DOCKER = /bin/systemctl restart, /bin/docker  %tester secsrv=(root) NOPASSWD: DOCKER  
EOF  
  fi  
...
получаем пустой файл test, т.к. вначале выполняется перенаправление и файл назначения обнуляется, а потом выполняется команда tail


___

 - Задача перенаправить только стандартный поток ошибок команды ls на вход команды grep.

при таком запуске команд, с испрользованием перенаправлений, как указано ниже, перенаправления выполняются слева на право, pipe(|) выполняется первым :

        ls -l asdf 2>&1 >/dev/null | grep asdf

1. (|) перенаправить стандартный поток вывода через pipe на стандартный поток ввода команды grep
2. (2>&1) перенаправить стандартный поток ошибок туда, куда на данный момент направлен стандартный поток вывода (&1 - указатель на место куда направлен stdout)
3. (>/dev/null) а стандартный поток вывода, ещё раз перенаправить в /dev/null.

**В итоге перенаправляем stderr команды ls, на вход команды grep, а stdout команды ls отправляем в /dev/null**
___

##### Использование дополнительных дискрипторов и команды exec.

Операция перенаправления обычно носит временный характер, то есть она действительна только для текущей команды. Если необходимо задействовать постоянное перенаправление в/из файла можно  использовать команду exec. Применимо ко всем файловым дискрипторам (0,1,2...)

Формат:

    **exec "номер дискриптора (доступны 3-9)" > <filename>**

      [andrey@nix64amd Vim]$ exec 3>test_out - перенаправим вывод файлового дискриптора 3 в файл <filename> . Результат смотрим командой **lsof -a -p $$ -d 0-255**

              ......
              bash    3555558 andrey    3w   REG   8,33     2260 156912179 /storage2/Study/Vim/test_out
              ......



Сохранить текущие значения стандартных файловых дискрипторов, например стандартный вывод:

      [andrey@nix64amd Vim]$ exec 6>&1

      ....
      bash    3555558 andrey    1w   CHR 136,29      0t0        32 /dev/pts/29        (/dev/pts/29 -- текущая консоль в которой работает процесс)
      bash    3555558 andrey    6w   CHR 136,29      0t0        32 /dev/pts/29
      ....
Мы перенаправили файловый дискриптор с номером 6, туда куда ссылается файловый дискриптор с номером 1 (стандартный вывод - /dev/pts/29). Далее можем постоянно  перенаправить стандартный вывод, в нужный нам файл  **exec 1>/storage2/Study/Vim/stdout2_test**, чтоб при выполнение скрипта, все данные писались в этот файл. По окончании работы скрипта, восстановим вывод 1 дискриптора:

      [andrey@nix64amd Vim]$ exec 1>&6
      ....
      bash    3555558 andrey    1w   CHR 136,29      0t0        32 /dev/pts/29
...
вернёт имя специального файла, прочитав который можно получить вывод КОМАНДЫ.
      ...
      [andrey@nix64amd Download]$ echo <(ls -l)
      /dev/fd/63
...
*   **set              :**    <---- _установка атрибутов переменных оболочки_

*   **declare       :**    <---- _установка значений переменных и управление атрибутами переменных_

*   **export         :**  _Все объявленные с помощью нее переменные экспортируются во внешнее окружение среды и будут доступны всем дочерним скриптам и программам. С помощью опции -p вы можете посмотреть экспортированные на данный момент переменные._


_Переменные окружения — это переменные, определяемые для текущей оболочки и наследуемые любыми дочерними оболочками или процессами. Переменные окружения используются для передачи информации процессам, которые запущены в оболочке._

_Переменные оболочки — это переменные, содержащиеся исключительно в оболочке, внутри которой они были заданы или определены. Часто они используются для отслеживания кратковременных данных, например текущего рабочего каталога._
...
Запуск программы на основе переменных окружения. Могут быть установлены свои переменные, или модифицированы существующие. Основная идея - улучшение переносимости. Не гарантируется, что на различных системах исполняемый файл будет лежать по пути, который указан в shebang. Использование env позволяет снизить этот риск за счет запуска команды на основе данных из переменной среды PATH. Более того, если по каким-либо причинам вместо стандартного исполняемого файла пользователь хочет использовать свой, то ему достаточно добавить путь к этому файлу в PATH без необходимости исправления скриптов
...
<details>
    ...
    где:
            -w 20 <--- при повторном запуске flock ожидает освобождения блокировки от первого запуска указанное значение сек. Если блокировка не снята, второй  процесс завершается.
            -n    <---- Немедленное завершение процесса, если блокировка не может быть получена. Т.е второй запуск завершается сразу, без ожиданя возможного завршения первого запуска (/usr/bin/flock -n -E 20 /var/tmp/log/hw_bash.lock -c ./hw_genlog.sh). Код завршения будет - 20
            -x    <---- эксклюзивная блокировка (блокировка записи), может быть другой вариант блокировки ( блокировка чтения)
            /var/tmp/log/hw_genlog.lock <---   при вызове надо будет указать файл блокировки, должен находится в директории с правами на запись.
...
*   **nohup         :** _Cохранить запущенные процессы после прекращения работы терминала_
    ...
    Либо, если процесс уже запущен в фоне, отвязать его от сессии можно командой disown с идентификатором ID (узнать ID - команда: jobs) нужной задачи.
...
*  **expand          :** _Команда замены символов табуляции на пробелы в файле_
*  **unexpand       :**  _команда замены символов пробела на символы табуляции в файле_

___

Выполнить команду из **sudo**  в _subshell_:
...
*   **top                 :**   _Просмотр параметров процессов в интерактивном режиме_
...
#### Команды для работы со строкаим
___

*   **paste         :** _Команда объединения строк из файлов_
...
*    **sort      :** _Команда сортировки файлов_
*    **uniq      :** _Команда предназначена для поиска одинаковых строк в массивах текста._
...
*    **comm     :** _Команда построчного сравнения файлов_
...
>   **Работает со строкой**
>
>   **_[ addr [ ,addr ] ] cmd '[ args ]'_**  _ОБЩИЙ СИНТАКСИС КОМАНДЫ SED_


<details>
    <summary>Потоковый текстовый редактор "sed". Примеры использования </summary>
...
**Проверки для файлов:**
    ...
    [[ -b ${file} ]] — файл блочное устройство
    [[ -c ${file} ]] — файл символное устройство
    [[ -h ${file} ]] — файл символическая ссылка
    [[ -v varname ]] - переменная в shell определена, указывается именно **"-v varname"**, без раскрытия значения переменной

        if [[ -z "$test"]]
              then
                  :    <---- Выражение «:» является нулевым оператором в оболочке - поэтому оно ничего не делает в этом случае
              else
                  echo $test
        fi
___
...
*   _**SELECT**_ - Цикл выбора помогает нам организовать пользовательское меню. Он имеет почти тот же синтаксис, что и цикл for
      ...
      Пример: пользователю задается вопрос, какой диспетчер пакетов он хотел бы использовать. Затем он спросит, какой пакет мы хотим установить, и, наконец, приступит к его установке:
