...
>  Для удаления всех закомментированных строк (например начинающихся с #), а так же всех пустых строк выполните:
    ...
    sed -e 's/#.*//;/^$/d' testfile.txt     <----  Вначале заменяем все строки с коментариями на пустые, а потом пустые строки удаляем. Аргументы для замены и удаления разделяются ";" - 's/#.*//;/^$/d' Хотя можно было сразу удалять все строки с коментариями (sed -e '/^#/d' test.log > test1.log). 
    
    (sed -e '/^#/d' test.log > test1.log)
